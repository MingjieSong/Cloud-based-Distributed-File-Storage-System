...
func ClientSync(client RPCClient) {
	...
	blockStoreServerAddr := &BlockStoreAddr{}
	blockAddrErr := client.GetBlockStoreAddr(&blockStoreServerAddr.Addr)
	if blockAddrErr != nil {
	...
	*/
	...
	for _, filename := range filesToDownload {
		newMeta, err := DownloadFile(client, filename, remoteIndex[filename], baseDir, blockStoreServerAddr.Addr)
		if err != nil {
	...
	for _, filename := range filesToUpload {
		newMeta, err := UploadFile(client, filename, localIndex[filename], baseDir, blockStoreServerAddr.Addr, blockSize)
		if err != nil {
			log.Printf("Received error downloading file %v", err)
		} else {
			localIndex[filename] = &newMeta
	...
	for _, filename := range filesDeleted {
		newMeta, err := DeleteFile(client, filename, remoteIndex, localIndex[filename], baseDir, blockStoreServerAddr.Addr)
		if err != nil {
...
func DownloadFile(client RPCClient, filename string, remoteMeta *FileMetaData, baseDir string, blockAdd string) (FileMetaData, error) {
...
func UploadFile(client RPCClient, filename string, localMeta *FileMetaData,
	baseDir string, blockAdd string, blockSize int) (FileMetaData, error) {
	path := ConcatPath(baseDir, filename)
	...
	err = client.HasBlocks(localMeta.BlockHashList, blockAdd, &unchangedBlocks)
	if err != nil {
	...
	if serverVersion == -1 {
		...
		newMeta, err := handleConflict(client, remoteIndex, filename, baseDir, blockAdd)
		if err != nil {
		...
		for hashString, block := range hashStringToBlock {
			flag, _ := ContainString(hashString, unchangedBlocks)
			if !flag {
				success := &Success{Flag: false}
				err = client.PutBlock(block, blockAdd, &success.Flag)
				if err != nil {
...
func DeleteFile(client RPCClient, filename string, remoteIndex map[string]*FileMetaData, local_meta *FileMetaData,
	baseDir string, blockAdd string) (FileMetaData, error) {
...
func handleConflict(client RPCClient, remoteIndex map[string]*FileMetaData, filename string, baseDir string, blockAdd string) (FileMetaData, error) {
	err := client.GetFileInfoMap(&remoteIndex)
	if err != nil {
		log.Printf("Received error in GetFileInfoMap %v", err)
	}
